...
export async function usersGET(req: Request, env: Env): Promise<Response> {
    const repo = new Repository(new D1Driver(env.D1));
    const id = getEntityId(req);
    const rows = id ? await repo.get("users", id) : await repo.getAll("users");
    if (id && rows === null) {
...
export async function usersPOST(req: Request, env: Env): Promise<Response> {
    const repo = new Repository(new D1Driver(env.D1));
    ...
    await repo.save("users", record, ["id"]);
    return Response.json({ success: true, entity: "users", id: record.id }, { status: 200 });
}

export async function usersPUT(req: Request, env: Env): Promise<Response> {
    const repo = new Repository(new D1Driver(env.D1));
    const payload = await readJsonBody(req);
    const id = (payload.id as string | undefined) ?? getEntityId(req);
    if (!id) return Response.json({ error: "id is required" }, { status: 400 });
    const result = await repo.update("users", { ...payload, updated_at: new Date().toISOString() }, id);
    return Response.json({ success: true, entity: "users", id }, { status: result.success ? 200 : 400 });
}

export async function usersDELETE(req: Request, env: Env): Promise<Response> {
    const repo = new Repository(new D1Driver(env.D1));
    const id = getEntityId(req);
    if (!id) return Response.json({ error: "id is required" }, { status: 400 });
    const result = await repo.delete("users", id);
